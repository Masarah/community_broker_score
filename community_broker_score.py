...
def detect_brokering_edges(nodes, edges):
    ...
    edges_B_with_cid = edges_A_with_cid_renamed.merge(nodes, left_on='B', right_on= 'id', how='inner').drop(columns=['id'])
...
def detect_community_brokers(nodes, edges):
    ...
    community_brokers_list_duplicates = pd.concat([those_who_are_CB_in_A, those_who_are_CB_in_B]) #without the drop duplicates
    community_brokers_list_count = community_brokers_list_duplicates.groupby('id').count().reset_index().rename(columns={'community_broker':'n_community_broker'})
    ...
    nodes_with_characteristics['n_community_broker'] = nodes_with_characteristics['n_community_broker'].fillna(0)
...
def find_community_characteristics(nodes, edges):
    ...
    group_nodes_per_community = nodes_without_count_var.groupby('community_id').agg({'id': 'count', 'community_broker': 'sum'}).rename(columns = {'id':'community_n_people'}).reset_index()

    # Create a graph object and set community_id as nodes attributes 
    graph = nx.from_pandas_edgelist(edges, source='A', target='B')
    extra_community_id_as_attribute = pd.Series(nodes_with_characteristics.community_id.values, index=nodes_with_characteristics.id).to_dict()
    ...
    for each_community_id in nodes_with_characteristics.community_id.unique(): 
        nodes_per_community = nodes_with_characteristics.loc[nodes_with_characteristics['community_id'] == each_community_id]
    ...
    communities = pd.DataFrame(cohesion_score).merge(group_nodes_per_community, on='community_id', how='outer').rename(columns={'community_broker':'community_n_brokers'})
...
def local_community_broker_score(nodes, edges):
    ...
    edges_only_community_bridge_inversed = edges_only_community_bridge.rename(columns={'A':'B',
                                                                                       'B':'A',
                                                                                       'community_id_A': 'community_id_B',
                                                                                       'community_id_B':'community_id_A'})
    edges_only_community_bridge_inversed_column_ordered = edges_only_community_bridge_inversed[['A', 'B', 'community_id_A', 'community_id_B', 'community_broker' ]]
    
    # Drop duplicates because sometimes one community broker bridges the same community many times 
    all_broker_ties = pd.concat([edges_only_community_bridge, edges_only_community_bridge_inversed]).drop_duplicates()                                                                  
    all_broker_ties_unique_in_one_column = all_broker_ties[['A', 'community_id_A', 'community_id_B', 'community_broker']].rename(columns={'A':'id'})                                                                  
    
    
    # Detect cobridgers for based on community source (A) towards community target (B) for each broker tie 
    n_CB_A_towards_B = all_broker_ties_unique_in_one_column.groupby(['community_id_A', 'community_id_B']).agg({'id':'count'}).rename(columns = {'id':'n_cobrokers'}).reset_index()
    
    # Merge with all_broker_ties_unique_in_one_column to get info alltogether
    all_broker_ties_info = all_broker_ties_unique_in_one_column.merge(n_CB_A_towards_B, on=['community_id_A', 'community_id_B'], how='outer')

    
    # Since we will merge info from cohesion dataframe, keep only interesting columns from the dataframe 
    interesting_columns_communities = communities[['community_id', 'community_cohesion', 'community_n_people']]


    # Create large dataframe with all info to calculate local community broker score for each briding tie created 
    info_community_A = all_broker_ties_info.merge(interesting_columns_communities, left_on='community_id_A', right_on='community_id', how='outer').rename(columns={'community_cohesion': 'community_cohesion_CA', 'community_n_people':'community_n_people_CA'}).drop('community_id', axis=1)
    info_community_AB = info_community_A.merge(interesting_columns_communities, left_on='community_id_B', right_on='community_id', how='outer').rename(columns={'community_cohesion': 'community_cohesion_CB', 'community_n_people':'community_n_people_CB'}).drop('community_id', axis=1)
    
    # Calculate Score for community targeted (CB) 
    
    info_community_AB_with_score_CB = info_community_AB.assign(score_CB = info_community_AB.community_n_people_CB / (info_community_AB.community_cohesion_CB * np.sqrt(info_community_AB.n_cobrokers))) 
    info_community_AB_with_scores_CB_summed = info_community_AB_with_score_CB.groupby('id').agg({'id': 'first', 'community_cohesion_CA':'first', 'community_n_people_CA':'first', 'score_CB':'sum'})
    
    # Calculate score for boker's own community 
    info_community_AB_with_scores_CB_CA = info_community_AB_with_scores_CB_summed.assign(score_CA = info_community_AB_with_scores_CB_summed.community_n_people_CA / info_community_AB_with_scores_CB_summed.community_cohesion_CA)

    # Calculate final score 
    final_scores = info_community_AB_with_scores_CB_CA.assign(community_broker_score = (info_community_AB_with_scores_CB_CA.score_CB + info_community_AB_with_scores_CB_CA.score_CA)).reset_index(drop=True)
    
    # Merge final score with nodes dataframe
    final_scores_relevant_columns_for_merge = final_scores[['id', 'community_broker_score']]
    nodes_with_broker_score = nodes_with_characteristics.merge(final_scores_relevant_columns_for_merge, on='id', how='left')
    nodes_with_broker_score['community_broker_score'] = nodes_with_broker_score['community_broker_score'].fillna(0)
